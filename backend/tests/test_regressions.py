...
def test_legacy_introduced_but_unreviewed_queue_is_capped_without_losing_reviews(
    ...
    with TestClient(app) as client:
        ...
        client.post(
            ...
            json={"outcome": "correct", "queue_entry_id": first["queue_entry_id"]},
        )
        with database.connection() as db:
            for i in range(146):
                db.execute(
                    "INSERT INTO cards(id,repertoire_id,kind,start_fen,moves_json,state,due_date,introduced_at) VALUES(?,?,'prefix',?,'[\"e2e4\"]','learning',?,?)",
                    (
                        f"legacy-{i}",
                        imported["repertoire_id"],
                        first["start_fen"],
...
def test_completed_queue_entry_is_idempotent_and_reinforcement_schedules_into_the_future(
    ...
    with TestClient(app) as client:
        ...
        scheduled = client.post(
            ...
            json={
                "outcome": "correct",
                "queue_entry_id": reinforcement["queue_entry_id"],
            },
        ).json()
...
def test_deletion_isolates_records_sharing_a_source_filename(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        for side in ("white", "black"):
            result = client.post(
                "/api/imports/pgn",
                files={"file": ("Tempo examples.pgn", PGN)},
                data={"trained_color": side, "initial_depth": 2},
            ).json()
            ids.append(result["repertoire_id"])
        assert ids[0] != ids[1]
        client.delete(f"/api/repertoires/{ids[0]}")
        assert [
            r["id"] for r in client.get("/api/repertoires").json()["repertoires"]
        ] == [ids[1]]
        assert [
            r["id"] for r in client.get("/api/repertoires").json()["repertoires"]
        ] == [ids[1]]
        assert all(
            line["repertoire_id"] == ids[1]
            for line in client.get("/api/repertoire/lines").json()["lines"]
        )


def test_delete_branch_prefix_removes_nimzo_descendants_and_preserves_qgd(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(database, "DB_PATH", tmp_path / "tempo.db")
    with TestClient(app) as client:
        imported = client.post(
            "/api/imports/pgn",
            files={"file": ("branches.pgn", BRANCH_PGN)},
        ...
        lines_before = [
            line
            for line in client.get("/api/repertoire/lines").json()["lines"]
            if line["repertoire_id"] == repertoire_id
        ]
        assert any(line["moves"][:2] == ["d2d4", "d7d5"] for line in lines_before)
        assert any(line["moves"][:2] == ["d2d4", "g8f6"] for line in lines_before)

        removed = client.post(
            "/api/repertoire/branches/remove",
            json={
                "repertoire_id": repertoire_id,
                "starting_fen": lines_before[0]["start_fen"],
                "moves": ["d2d4", "g8f6"],
            },
        )
        assert removed.status_code == 200
        assert removed.json()["deleted_line_count"] >= 1
...
def test_delete_branch_rebuilds_missing_retained_cards_without_server_error(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(database, "DB_PATH", tmp_path / "tempo.db")
    rebuild_pgn = b'[Event "QGD"]\n[White "andy"]\n[Black "opponent"]\n[Result "*"]\n\n1. d4 d5 2. c4 *\n\n[Event "Nimzo"]\n[White "andy"]\n[Black "opponent"]\n[Result "*"]\n\n1. d4 Nf6 2. c4 e6 3. Nc3 *'
    with TestClient(app) as client:
        imported = client.post(
            "/api/imports/pgn",
            files={"file": ("branches-rebuild.pgn", rebuild_pgn)},
            data={"trained_color": "white", "initial_depth": 6},
        ).json()
        repertoire_id = imported["repertoire_id"]
        lines = [
        ...
        with database.connection() as db:
            db.execute(
                "DELETE FROM repertoire_cards WHERE repertoire_id=?",
                (repertoire_id,),
            )
            db.execute("DELETE FROM cards WHERE repertoire_id=?", (repertoire_id,))

        removed = client.post(
            "/api/repertoire/branches/remove",
            json={
                "repertoire_id": repertoire_id,
                "starting_fen": lines[0]["start_fen"],
                "moves": ["d2d4", "g8f6"],
            },
        )
        assert removed.status_code == 200
...
def test_light_discovery_uses_settings_and_never_returns_to_light_after_a_lapse(
    tmp_path, monkeypatch
):
    ...
    with TestClient(app) as client:
        settings = client.get("/api/settings").json()
        settings["light_first_interval_days"] = 10
        client.put("/api/settings", json=settings)
        payload = {
            "puzzle_id": "one",
            "deck_id": "fork-easy",
        ...
        assert (
            light["mode"] == "light"
            and light["next_due"] == (date.today() + timedelta(days=10)).isoformat()
        )
        client.post(
            "/api/tactics/attempt", json={**payload, "correct": False, "clean": False}
        )
        clean_again = client.post("/api/tactics/attempt", json=payload).json()
        assert (
            clean_again["mode"] == "normal"
            and clean_again["next_due"] == date.today().isoformat()
        )


def test_local_sync_persists_errors_and_success_without_sample_fallback(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(database, "DB_PATH", tmp_path / "tempo.db")
    ...
    with TestClient(app) as client:
        ...
        assert (
            ...
            and state["last_error"] == "Lichess username not found"
        )
        ...
        assert (
            client.post(
                f"/api/games/{quote(game_id, safe='')}/analysis",
                json={
                    "evaluations": [{"ply": 0, "before_cp": 0, "after_cp": -150}],
                    "depth": 6,
                },
            ).status_code
            == 200
        )
        assert int(requests[-1].url.params["since"]) > int(
            requests[-2].url.params["since"]
        )
        assert client.get("/api/games/sync/status").json()["providers"][0][
            "last_success_at"
        ]
...
def test_again_reappears_after_four_other_entries(tmp_path, monkeypatch):
    ...
    with TestClient(app) as client:
        ...
        with database.connection() as db:
            for i in range(5):
                db.execute(
                    "INSERT INTO cards(id,repertoire_id,kind,start_fen,moves_json,state,due_date,introduced_at) VALUES(?,?,'prefix',?,'[\"e2e4\"]','learning',?,'2020-01-01')",
                    (
                        f"other-{i}",
                        first["repertoire_id"],
                        first["start_fen"],
                ...
                db.execute(
                    "INSERT INTO daily_queue(queue_date,card_id,position) VALUES(?,?,?)",
                    (date.today().isoformat(), f"other-{i}", i + 1),
                )
        client.post(
            f"/api/cards/{first['id']}/review",
            json={"outcome": "again", "queue_entry_id": first["queue_entry_id"]},
        )
...
def test_game_mistakes_respect_arbitrary_fen_side_to_move():
    ...
    result = classify_swings(
        [{"ply": 0, "before_cp": 0, "after_cp": 150}], "black", starting_color="black"
    )
    assert result["major_mistake_ply"] == 0


def test_help_failure_survives_reload_and_cannot_be_graded_as_a_clean_solve(
    tmp_path, monkeypatch
):
    monkeypatch.setattr(database, "DB_PATH", tmp_path / "tempo.db")
    with TestClient(app) as client:
        ...
        for _ in range(2):
            assert (
                client.post(
                    f"/api/queue/entries/{card['queue_entry_id']}/fail"
                ).status_code
                == 200
            )
        ...
        client.post(
            ...
            json={"outcome": "correct", "queue_entry_id": card["queue_entry_id"]},
        )
        with database.connection() as db:
            ...
            assert (
                db.execute(
                    "SELECT first_correct_at FROM cards WHERE id=?", (card["id"],)
                ).fetchone()[0]
                is None
            )
        next_attempt = client.get("/api/queue/today").json()["cards"][0]
        assert (
            next_attempt["attempt_failed"] == 0
            and next_attempt["queue_entry_id"] != card["queue_entry_id"]
        )
...
def test_unfinished_unreviewed_cards_do_not_bypass_tomorrows_new_card_limit(
    ...
    with TestClient(app) as client:
        ...
        with database.connection() as db:
            for i in range(5):
                db.execute(
                    "INSERT INTO cards(id,repertoire_id,kind,start_fen,moves_json,state,due_date) VALUES(?,?,'prefix',?,'[\"e2e4\"]','new',?)",
                    (
                        f"new-{i}",
                        first["repertoire_id"],
                        first["start_fen"],
