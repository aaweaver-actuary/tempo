...
| Cumulative branch cards repeatedly test shared opening decisions and delay descendants | `test_lines_sharing_a_prefix_materialize_one_card_per_shared_decision`; `test_branch_divergence_creates_distinct_cards_only_after_the_divergence`; `test_identical_context_segments_share_one_global_schedule_across_repertoires`; `test_distinct_opponent_cues_to_the_same_position_remain_distinct_cards`; `test_descendant_requires_a_mature_parent`; `test_any_mature_incoming_path_unlocks_a_transposed_descendant`; `test_shared_card_counts_once_while_respecting_repertoire_admission_limits` |
| Opening graph rebuilds can overwrite newer input, hold SQLite during traversal, or damage queue/history state | `test_repeated_graph_rebuilds_are_generation_guarded_and_idempotent`; `test_graph_rebuild_holds_no_sqlite_connection_during_chess_traversal`; `test_cumulative_migration_preserves_reviews_and_caps_seeded_stability`; `test_failed_seed_verification_does_not_revoke_introduced_descendants`; `test_graph_publication_never_rewrites_completed_queue_attempts`; `test_canonical_decision_cards_do_not_offer_prefix_splitting`; `test_branch_removal_unlinks_or_archives_only_unreferenced_decision_cards` |
