...
it("Builder comparison exposes database connection and source status rather than unexplained blanks", () => {
  vi.stubGlobal("fetch", vi.fn(async () => Response.json({ lines: [], annotations: [] })));
  render(<BuilderView imported={[]} settings={new Settings()} theme="brown" pieceSet="cburnett" />);
  const panel=screen.getByRole("table", { name: "Move source comparison" }).closest("section")!;
  expect(panel.querySelector(".comparison-connect")?.textContent).toBe("Connect databases");
...
it("builder flip preserves repertoire identity and history across remounts", async () => {
  vi.stubGlobal("fetch", vi.fn(async (input) => Response.json(String(input).includes("/repertoire/lines") ? { lines: [{ id:"black-line", repertoire_id:"black-repertoire", repertoire_name:"Gambits", name:"", trained_color:"black", start_fen:new Chess().fen(), moves:["d2d4","g8f6","0000"] }] } : { annotations: [] })));
