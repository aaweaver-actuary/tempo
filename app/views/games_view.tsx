...
export function GamesView({ onAnalyze, onSettings, onSync, syncState, theme, pieceSet }: {
  ...
  const [engineOn, setEngineOn] = useState(() => localStorage.getItem("tempo-games-engine-on") !== "false");
  const [engineText, setEngineText] = useState("");
  const [error, setError] = useState("");
  const [scanStatus, setScanStatus] = useState("");
  const [lines, setLines] = useState<AnalysisLine[]>([]);
  const [filters, setFilters] = useState(() => ({ source: "All", status: "All", color: "All", speed: "All", result: "All", from: new Date(Date.now() - 90 * 86400000).toISOString().slice(0, 10) }));
  const selected = records.find((game) => game.id === selectedId) ?? records[0];
  const gameFen = selected ? fenAfterMoves(selected.moves, Math.min(cursor, selected.moves.length), selected.startFen) : STANDARD_FEN;
  const gameLast = selected && cursor ? convertSanToUci(selected.moves, selected.startFen)[cursor - 1] : undefined;
  const positions = useMemo(() => indexRepertoirePositions(lines), [lines]);
  const shapes = positions.filter((position) => canonicalFenKey(position.fen) === canonicalFenKey(gameFen) && position.nextUci && (!selected?.repertoireId || position.repertoireId === selected.repertoireId)).map((position) => ({ orig: position.nextUci!.slice(0, 2), dest: position.nextUci!.slice(2, 4), brush: "yellow" } as DrawShape));
  const games = records.filter((game) => game.date >= filters.from && (["source", "status", "color", "speed", "result"] as const).every((field) => filters[field] === "All" || filters[field] === game[field]));
  const loadGames = useCallback(async () => {
    ...
    try {
      ...
      const loaded = body.games.map(importGameAndReformatToGameViewRecord).filter((game): game is GameViewRecord => Boolean(game));
      setRecords(loaded);
      ...
      setError("");
    } catch (reason) { setError(reason instanceof Error ? reason.message : "Could not load games."); }
  }, [local]);
  useEffect(() => { queueMicrotask(() => void loadGames()); }, [loadGames, syncState.lastSuccess]);
  useEffect(() => {
    if (!local) return;
  ...
  useEffect(() => {
    ...
    queueMicrotask(() => { if (!controller.signal.aborted) setScanStatus(`Scanning ${pending.opening}…`); });
    void scanGame(pending.startFen, pending.moves, pending.color, (fen) => analyzeWithStockfish(fen, 6), controller.signal).then(async (evaluations) => {
      if (controller.signal.aborted) return;
      const response = await fetch(`${API_URL}/api/games/${encodeURIComponent(pending.id)}/analysis`, { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ evaluations, depth: 6 }) });
      if (!response.ok) throw new Error("Could not save the game scan.");
      await loadGames();
      setScanStatus("");
    }).catch((reason) => { if (!controller.signal.aborted) setScanStatus(`Game scan failed: ${reason instanceof Error ? reason.message : "retry by reopening Games"}`); });
    return () => controller.abort();
  }, [local, pending, loadGames]);
  useEffect(() => {
    let active = true;
    if (!engineOn || !selected) return;
    queueMicrotask(() => { if (active) setEngineText("Analyzing…"); });
    void analyzeWithStockfish(gameFen, 12).then((moves) => { if (active) setEngineText(moves[0] ? `${moves[0].san} · ${moves[0].score}` : "Terminal position"); }).catch((reason) => { if (active) setEngineText(`Engine error: ${reason.message}`); });
    return () => { active = false; };
  }, [engineOn, gameFen, selected]);
  useEffect(() => {
    const onKey = (event: KeyboardEvent) => {
      if ((event.target as HTMLElement)?.matches('input,textarea,select,[contenteditable="true"]') || document.querySelector('[role="dialog"]')) return;
      const length = selected?.moves.length ?? 0;
      if (event.key === "ArrowLeft") { event.preventDefault(); setCursor((value) => Math.max(0, value - 1)); }
      if (event.key === "ArrowRight") { event.preventDefault(); setCursor((value) => Math.min(length, value + 1)); }
      if (["Home", "ArrowUp"].includes(event.key)) { event.preventDefault(); setCursor(0); }
      if (["End", "ArrowDown"].includes(event.key)) { event.preventDefault(); setCursor(length); }
    };
  ...
  return <section className="games-page" id="games">
    <div className="page-heading compact"><div><h1>Games{!local ? " · Demo" : ""}</h1><p>{syncState.lastSuccess ? `Last sync ${new Date(syncState.lastSuccess).toLocaleString()}` : "No successful sync yet"}</p></div>
      {local && <button className="primary-button sync-button" onClick={onSync} disabled={syncState.syncing}>{syncState.syncing && <i />}{syncState.syncing ? "Syncing games" : "↻ Sync games"}</button>}
    </div>
    {(error || syncState.error) && <p role="alert">{error || syncState.error} <button onClick={onSettings}>Account settings</button></p>}
    {!local && <p>Sample comparisons. Import and sync your games in <a href="https://github.com/aaweaver-actuary/tempo#running-locally">full local Tempo</a>.</p>}
    <div className="game-review">
      <div className="game-board"><Chessboard fen={gameFen} lastMove={gameLast ? [gameLast.slice(0, 2), gameLast.slice(2, 4)] : undefined} shapes={shapes} locked showHint={false} theme={theme} pieceSet={pieceSet} onMove={() => undefined} orientation={selected?.color} />
        <div className="board-tools"><button onClick={() => setCursor((value) => Math.max(0, value - 1))}>← Back</button><button onClick={() => setCursor((value) => Math.min(selected?.moves.length ?? 0, value + 1))}>Forward →</button><button disabled={!selected} onClick={() => setCursor(selected?.flagPly ?? 0)}>⚑ First mistake</button><button className={engineOn ? "active" : ""} onClick={() => setEngineOn((value) => { localStorage.setItem("tempo-games-engine-on", String(!value)); return !value; })}>Stockfish</button></div>
      </div>
      <div className="game-side-scroll">
        <aside className="game-inspector"><h2>{selected?.opening ?? "No games imported"}</h2><strong>{selected?.flag}</strong>{engineOn && <p>{engineText}</p>}{scanStatus && <p role="status">{scanStatus}</p>}
